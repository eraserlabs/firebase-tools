      ...
      `,
        ...
        tests.forEach(({ description, input, want }) => {
            it(description, () => {
                ...
                (0, chai_1.expect)(envs).to.deep.equal(want);
                (0, chai_1.expect)(errors).to.be.empty;
            });
        });
        it("should catch invalid lines", () => {
            (0, chai_1.expect)(env.parse(`
BAR###
FOO=foo
// not a comment
=missing key
`)).to.deep.equal({
    ...
    describe("writeUserEnvs", () => {
        const createEnvFiles = (sourceDir, envs) => {
        ...
        afterEach(() => {
            (0, rimraf_1.sync)(tmpdir);
            (0, chai_1.expect)(() => {
                fs.statSync(tmpdir);
            }).to.throw();
        });
        it("never affects the filesystem if the list of keys to write is empty", () => {
            env.writeUserEnvs({}, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir });
            env.writeUserEnvs({}, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir, isEmulator: true });
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.alias"))).to.throw;
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.project"))).to.throw;
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.local"))).to.throw;
        });
        it("touches .env.projectId if it doesn't already exist", () => {
            env.writeUserEnvs({ FOO: "bar" }, { projectId: "project", functionsSource: tmpdir });
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.alias"))).to.throw;
            (0, chai_1.expect)(!!fs.statSync(path.join(tmpdir, ".env.project"))).to.be.true;
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.local"))).to.throw;
        });
        it("touches .env.local if it doesn't already exist in emulator mode", () => {
            ...
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.alias"))).to.throw;
            (0, chai_1.expect)(() => fs.statSync(path.join(tmpdir, ".env.project"))).to.throw;
            (0, chai_1.expect)(!!fs.statSync(path.join(tmpdir, ".env.local"))).to.be.true;
        });
        it("throws if asked to write a key that already exists in .env.projectId", () => {
            createEnvFiles(tmpdir, {
                [".env.project"]: "FOO=foo",
            });
            (0, chai_1.expect)(() => env.writeUserEnvs({ FOO: "bar" }, { projectId: "project", functionsSource: tmpdir })).to.throw(error_1.FirebaseError);
        });
        it("is fine writing a key that already exists in .env.projectId but not .env.local, in emulator mode", () => {
            ...
            env.writeUserEnvs({ FOO: "bar" }, { projectId: "project", functionsSource: tmpdir, isEmulator: true });
            (0, chai_1.expect)(env.loadUserEnvs({
                projectId: "project",
                projectAlias: "alias",
        ...
        it("throws if asked to write a key that already exists in any .env", () => {
            ...
            (0, chai_1.expect)(() => env.writeUserEnvs({ FOO: "baz" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir })).to.throw(error_1.FirebaseError);
        });
        it("is fine writing a key that already exists in any .env but not .env.local, in emulator mode", () => {
            createEnvFiles(tmpdir, {
                [".env"]: "FOO=bar",
            });
            env.writeUserEnvs({ FOO: "baz" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir, isEmulator: true });
            (0, chai_1.expect)(env.loadUserEnvs({
                projectId: "project",
                projectAlias: "alias",
                functionsSource: tmpdir,
        ...
        it("throws if asked to write a key that already exists in .env.local, in emulator mode", () => {
            ...
            (0, chai_1.expect)(() => env.writeUserEnvs({ ASDF: "bar" }, { projectId: "project", functionsSource: tmpdir, isEmulator: true })).to.throw(error_1.FirebaseError);
        });
        it("throws if asked to write a key that fails key format validation", () => {
            (0, chai_1.expect)(() => env.writeUserEnvs({ lowercase: "bar" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir })).to.throw(env.KeyValidationError);
            (0, chai_1.expect)(() => env.writeUserEnvs({ GCP_PROJECT: "bar" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir })).to.throw(env.KeyValidationError);
            (0, chai_1.expect)(() => env.writeUserEnvs({ FIREBASE_KEY: "bar" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir })).to.throw(env.KeyValidationError);
        });
        it("writes the specified key to a .env.projectId that it created", () => {
            env.writeUserEnvs({ FOO: "bar" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir });
            (0, chai_1.expect)(env.loadUserEnvs({ projectId: "project", projectAlias: "alias", functionsSource: tmpdir })["FOO"]).to.equal("bar");
        });
        ...
        it("writes multiple keys at once", () => {
            env.writeUserEnvs({ FOO: "foo", BAR: "bar" }, { projectId: "project", projectAlias: "alias", functionsSource: tmpdir });
            const envs = env.loadUserEnvs({
                projectId: "project",
                projectAlias: "alias",
                functionsSource: tmpdir,
            });
            (0, chai_1.expect)(envs["FOO"]).to.equal("foo");
            (0, chai_1.expect)(envs["BAR"]).to.equal("bar");
        });
        it("escapes special characters so that parse() can reverse them", () => {
            ...
            (0, chai_1.expect)(envs["ESCAPES"]).to.equal("\n\r\t\v");
            (0, chai_1.expect)(envs["WITH_SLASHES"]).to.equal("\n\\\r\\\t\\\v");
            (0, chai_1.expect)(envs["QUOTES"]).to.equal("'\"'");
        });
        it("shouldn't write anything if any of the keys fails key format validation", () => {
            try {
                env.writeUserEnvs({ FOO: "bar", lowercase: "bar" }, { projectId: "project", functionsSource: tmpdir });
            }
            catch (err) {
            }
            (0, chai_1.expect)(env.loadUserEnvs({ projectId: "project", functionsSource: tmpdir })["FOO"]).to.be
                .undefined;
        });
    });
    describe("loadUserEnvs", () => {
        ...
        it("loads nothing if .env files are missing", () => {
            (0, chai_1.expect)(env.loadUserEnvs(Object.assign(Object.assign({}, projectInfo), { functionsSource: tmpdir }))).to.be.deep.equal({});
        });
        ...
        it("loads envs, preferring ones from .env.<project> for emulators too", () => {
            ...
            (0, chai_1.expect)(env.loadUserEnvs(Object.assign(Object.assign({}, projectInfo), { functionsSource: tmpdir, isEmulator: true }))).to.be.deep.equal({
                FOO: "good",
                BAR: "bar",
            });
        });
        ...
        it("throws an error if both .env.<project> and .env.<alias> exists", () => {
            ...
            (0, chai_1.expect)(() => {
                env.loadUserEnvs(Object.assign(Object.assign({}, projectInfo), { functionsSource: tmpdir }));
