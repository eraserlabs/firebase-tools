"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const chai_1 = require("chai");
const sinon = require("sinon");
const backend = require("../../../../deploy/functions/backend");
const planner = require("../../../../deploy/functions/release/planner");
const deploymentTool = require("../../../../deploymentTool");
const utils = require("../../../../utils");
const v2events = require("../../../../functions/events/v2");
describe("planner", () => {
    let logLabeledBullet;
    function allowV2Upgrades() {
        sinon.stub(planner, "checkForV2Upgrade");
    }
    beforeEach(() => {
        logLabeledBullet = sinon.stub(utils, "logLabeledBullet");
    });
    afterEach(() => {
        sinon.verifyAndRestore();
    });
    function func(id, region, triggered = { httpsTrigger: {} }) {
        return Object.assign(Object.assign({ id,
            region }, triggered), { platform: "gcfv1", project: "project", runtime: "nodejs16", entryPoint: "function", environmentVariables: {} });
    }
    describe("calculateUpdate", () => {
        it("throws on illegal updates", () => {
            const httpsFunc = func("a", "b", { httpsTrigger: {} });
            const scheduleFunc = func("a", "b", { scheduleTrigger: {} });
            (0, chai_1.expect)(() => planner.calculateUpdate(httpsFunc, scheduleFunc)).to.throw();
        });
        it("knows to delete & recreate for v2 topic changes", () => {
            const original = Object.assign(Object.assign({}, func("a", "b", {
                eventTrigger: {
                    eventType: v2events.PUBSUB_PUBLISH_EVENT,
                    eventFilters: { topic: "topic" },
                    retry: false,
                },
            })), { platform: "gcfv2" });
            const changed = JSON.parse(JSON.stringify(original));
            if (backend.isEventTriggered(changed)) {
                changed.eventTrigger.eventFilters = { topic: "anotherTopic" };
            }
            (0, chai_1.expect)(planner.calculateUpdate(changed, original)).to.deep.equal({
                endpoint: changed,
                deleteAndRecreate: original,
            });
        });
        it("knows to delete & recreate for v1 to v2 scheduled function upgrades", () => {
            const original = Object.assign(Object.assign({}, func("a", "b", { scheduleTrigger: {} })), { platform: "gcfv1" });
            const changed = Object.assign(Object.assign({}, original), { platform: "gcfv2" });
            allowV2Upgrades();
            (0, chai_1.expect)(planner.calculateUpdate(changed, original)).to.deep.equal({
                endpoint: changed,
                deleteAndRecreate: original,
            });
        });
        it("knows to delete & recreate when trigger regions change", () => {
            const original = func("a", "b", {
                eventTrigger: {
                    eventType: "google.cloud.storage.object.v1.finalized",
                    eventFilters: { bucket: "my-bucket" },
                    region: "us-west1",
                    retry: false,
                },
            });
            original.platform = "gcfv2";
            const changed = func("a", "b", {
                eventTrigger: {
                    eventType: "google.cloud.storage.object.v1.finalzied",
                    eventFilters: { bucket: "my-bucket" },
                    region: "us",
                    retry: false,
                },
            });
            changed.platform = "gcfv2";
            allowV2Upgrades();
            (0, chai_1.expect)(planner.calculateUpdate(changed, original)).to.deep.equal({
                endpoint: changed,
                deleteAndRecreate: original,
            });
        });
        it("knows to upgrade in-place in the general case", () => {
            const v1Function = Object.assign(Object.assign({}, func("a", "b")), { platform: "gcfv1" });
            const v2Function = Object.assign(Object.assign({}, v1Function), { platform: "gcfv1", availableMemoryMb: 512 });
            (0, chai_1.expect)(planner.calculateUpdate(v2Function, v1Function)).to.deep.equal({
                endpoint: v2Function,
            });
        });
    });
    describe("calculateRegionalChanges", () => {
        it("passes a smoke test", () => {
            const created = func("created", "region");
            const updated = func("updated", "region");
            const deleted = func("deleted", "region");
            deleted.labels = deploymentTool.labels();
            const pantheon = func("pantheon", "region");
            const skipWant = func("skip", "region");
            skipWant.hash = "skip";
            const skipHave = func("skip", "region");
            skipHave.hash = "skip";
            const want = { created, updated, skip: skipWant };
            const have = { updated, deleted, pantheon, skip: skipHave };
            (0, chai_1.expect)(planner.calculateChangesets(want, have, (e) => e.region)).to.deep.equal({
                region: {
                    endpointsToCreate: [created],
                    endpointsToUpdate: [
                        {
                            endpoint: updated,
                        },
                    ],
                    endpointsToDelete: [deleted],
                    endpointsToSkip: [skipWant],
                },
            });
        });
        it("adds endpoints with matching hashes to skip list", () => {
            const updatedWant = func("updated", "region");
            const updatedHave = func("updated", "region");
            updatedWant.hash = "to_skip";
            updatedHave.hash = "to_skip";
            const want = { updated: updatedWant };
            const have = { updated: updatedHave };
            (0, chai_1.expect)(planner.calculateChangesets(want, have, (e) => e.region)).to.deep.equal({
                region: {
                    endpointsToCreate: [],
                    endpointsToUpdate: [],
                    endpointsToDelete: [],
                    endpointsToSkip: [updatedWant],
                },
            });
        });
        it("adds endpoints to update list if they dont have hashes", () => {
            const updatedWant = func("updated", "region");
            const updatedHave = func("updated", "region");
            const want = { updated: updatedWant };
            const have = { updated: updatedHave };
            (0, chai_1.expect)(planner.calculateChangesets(want, have, (e) => e.region)).to.deep.equal({
                region: {
                    endpointsToCreate: [],
                    endpointsToUpdate: [
                        {
                            endpoint: updatedWant,
                        },
                    ],
                    endpointsToDelete: [],
                    endpointsToSkip: [],
                },
            });
        });
        it("adds endpoints to update list if they have different hashes", () => {
            const updatedWant = func("updated", "region");
            const updatedHave = func("updated", "region");
            updatedWant.hash = "local";
            updatedHave.hash = "server";
            const want = { updated: updatedWant };
            const have = { updated: updatedHave };
            (0, chai_1.expect)(planner.calculateChangesets(want, have, (e) => e.region)).to.deep.equal({
                region: {
                    endpointsToCreate: [],
                    endpointsToUpdate: [
                        {
                            endpoint: updatedWant,
                        },
                    ],
                    endpointsToDelete: [],
                    endpointsToSkip: [],
                },
            });
        });
        it("does not add endpoints to skip list if not targeted for deploy", () => {
            const updatedWant = func("updated", "region");
            const updatedHave = func("updated", "region");
            updatedWant.hash = "to_skip";
            updatedHave.hash = "to_skip";
            updatedWant.targetedByOnly = true;
            const want = { updated: updatedWant };
            const have = { updated: updatedHave };
            (0, chai_1.expect)(planner.calculateChangesets(want, have, (e) => e.region)).to.deep.equal({
                region: {
                    endpointsToCreate: [],
                    endpointsToUpdate: [
                        {
                            endpoint: updatedWant,
                        },
                    ],
                    endpointsToDelete: [],
                    endpointsToSkip: [],
                },
            });
        });
        it("can be told to delete all functions", () => {
            const created = func("created", "region");
            const updated = func("updated", "region");
            const deleted = func("deleted", "region");
            deleted.labels = deploymentTool.labels();
            const pantheon = func("pantheon", "region");
            const want = { created, updated };
            const have = { updated, deleted, pantheon };
            (0, chai_1.expect)(planner.calculateChangesets(want, have, (e) => e.region, true)).to.deep.equal({
                region: {
                    endpointsToCreate: [created],
                    endpointsToUpdate: [
                        {
                            endpoint: updated,
                        },
                    ],
                    endpointsToDelete: [deleted, pantheon],
                    endpointsToSkip: [],
                },
            });
        });
    });
    describe("createDeploymentPlan", () => {
        const codebase = "default";
        it("groups deployment by region and memory", () => {
            const region1mem1Created = func("id1", "region1");
            const region1mem1Updated = func("id2", "region1");
            const region2mem1Created = func("id3", "region2");
            const region2mem2Updated = func("id4", "region2");
            region2mem2Updated.availableMemoryMb = 512;
            const region2mem2Deleted = func("id5", "region2");
            region2mem2Deleted.availableMemoryMb = 512;
            region2mem2Deleted.labels = deploymentTool.labels();
            const haveBackend = backend.of(region1mem1Updated, region2mem2Updated, region2mem2Deleted);
            const wantBackend = backend.of(region1mem1Created, region1mem1Updated, region2mem1Created, region2mem2Updated);
            (0, chai_1.expect)(planner.createDeploymentPlan({ wantBackend, haveBackend, codebase })).to.deep.equal({
                "default-region1-default": {
                    endpointsToCreate: [region1mem1Created],
                    endpointsToUpdate: [
                        {
                            endpoint: region1mem1Updated,
                        },
                    ],
                    endpointsToDelete: [],
                    endpointsToSkip: [],
                },
                "default-region2-default": {
                    endpointsToCreate: [region2mem1Created],
                    endpointsToUpdate: [],
                    endpointsToDelete: [],
                    endpointsToSkip: [],
                },
                "default-region2-512": {
                    endpointsToCreate: [],
                    endpointsToUpdate: [
                        {
                            endpoint: region2mem2Updated,
                        },
                    ],
                    endpointsToDelete: [region2mem2Deleted],
                    endpointsToSkip: [],
                },
            });
        });
        it("applies filters", () => {
            const group1Created = func("g1-created", "region");
            const group1Updated = func("g1-updated", "region");
            const group1Deleted = func("g1-deleted", "region");
            const group2Created = func("g2-created", "region");
            const group2Updated = func("g2-updated", "region");
            const group2Deleted = func("g2-deleted", "region");
            group1Deleted.labels = deploymentTool.labels();
            group2Deleted.labels = deploymentTool.labels();
            const wantBackend = backend.of(group1Updated, group1Created, group2Updated, group2Created);
            const haveBackend = backend.of(group1Updated, group1Deleted, group2Updated, group2Deleted);
            (0, chai_1.expect)(planner.createDeploymentPlan({
                wantBackend,
                haveBackend,
                codebase,
                filters: [{ codebase, idChunks: ["g1"] }],
            })).to.deep.equal({
                "default-region-default": {
                    endpointsToCreate: [group1Created],
                    endpointsToUpdate: [
                        {
                            endpoint: group1Updated,
                        },
                    ],
                    endpointsToDelete: [group1Deleted],
                    endpointsToSkip: [],
                },
            });
        });
        it("nudges users towards concurrency settings when upgrading and not setting", () => {
            const original = func("id", "region");
            original.platform = "gcfv1";
            const upgraded = Object.assign({}, original);
            upgraded.platform = "gcfv2";
            const haveBackend = backend.of(original);
            const wantBackend = backend.of(upgraded);
            allowV2Upgrades();
            planner.createDeploymentPlan({ wantBackend, haveBackend, codebase });
            (0, chai_1.expect)(logLabeledBullet).to.have.been.calledOnceWith("functions", sinon.match(/change this with the 'concurrency' option/));
        });
        it("does not warn users about concurrency when inappropriate", () => {
            allowV2Upgrades();
            const v2Function = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv2" });
            planner.createDeploymentPlan({
                wantBackend: backend.of(v2Function),
                haveBackend: backend.of(v2Function),
                codebase,
            });
            (0, chai_1.expect)(logLabeledBullet).to.not.have.been.called;
            const v1Function = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv1" });
            planner.createDeploymentPlan({
                wantBackend: backend.of(v1Function),
                haveBackend: backend.of(v1Function),
                codebase,
            });
            (0, chai_1.expect)(logLabeledBullet).to.not.have.been.called;
            const concurrencyUpgraded = Object.assign(Object.assign({}, v1Function), { platform: "gcfv2", concurrency: 80 });
            planner.createDeploymentPlan({
                wantBackend: backend.of(concurrencyUpgraded),
                haveBackend: backend.of(v1Function),
                codebase,
            });
            (0, chai_1.expect)(logLabeledBullet).to.not.have.been.called;
        });
    });
    describe("checkForIllegalUpdate", () => {
        it("prohibits upgrades from v1 to v2", () => {
            const have = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv1" });
            const want = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv2" });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).to.throw();
        });
        it("should throw if a https function would be changed into an event triggered function", () => {
            const want = func("a", "b", {
                eventTrigger: {
                    eventType: "google.pubsub.topic.publish",
                    eventFilters: {},
                    retry: false,
                },
            });
            const have = func("a", "b", { httpsTrigger: {} });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).to.throw();
        });
        it("should throw if a event triggered function would be changed into an https function", () => {
            const want = func("a", "b", { httpsTrigger: {} });
            const have = func("a", "b", {
                eventTrigger: {
                    eventType: "google.pubsub.topic.publish",
                    eventFilters: {},
                    retry: false,
                },
            });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).to.throw();
        });
        it("should throw if a scheduled trigger would change into an https function", () => {
            const want = func("a", "b");
            const have = func("a", "b", { scheduleTrigger: {} });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).to.throw();
        });
        it("should not throw if a event triggered function keeps the same trigger", () => {
            const eventTrigger = {
                eventType: "google.pubsub.topic.publish",
                eventFilters: {},
                retry: false,
            };
            const want = func("a", "b", { eventTrigger });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, want)).not.to.throw();
        });
        it("should not throw if a https function stays as a https function", () => {
            const want = func("a", "b");
            const have = func("a", "b");
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).not.to.throw();
        });
        it("should not throw if a scheduled function stays as a scheduled function", () => {
            const want = func("a", "b", { scheduleTrigger: {} });
            const have = func("a", "b", { scheduleTrigger: {} });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).not.to.throw();
        });
        it("should throw if a user downgrades from v2 to v1", () => {
            const want = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv1" });
            const have = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv2" });
            (0, chai_1.expect)(() => planner.checkForIllegalUpdate(want, have)).to.throw();
        });
    });
    it("detects changes to v2 pubsub topics", () => {
        const eventTrigger = {
            eventType: v2events.PUBSUB_PUBLISH_EVENT,
            eventFilters: { topic: "projects/p/topics/t" },
            retry: false,
        };
        let want = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv1" });
        let have = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv1" });
        (0, chai_1.expect)(planner.changedV2PubSubTopic(want, have)).to.be.false;
        want.platform = "gcfv2";
        (0, chai_1.expect)(planner.changedV2PubSubTopic(want, have)).to.be.false;
        have.platform = "gcfv2";
        (0, chai_1.expect)(planner.changedV2PubSubTopic(want, have)).to.be.false;
        want = Object.assign(Object.assign({}, func("id", "region", { eventTrigger })), { platform: "gcfv2" });
        (0, chai_1.expect)(planner.changedV2PubSubTopic(want, have)).to.be.false;
        have = Object.assign(Object.assign({}, func("id", "region", { eventTrigger })), { platform: "gcfv2" });
        (0, chai_1.expect)(planner.changedV2PubSubTopic(want, have)).to.be.false;
        want = JSON.parse(JSON.stringify(want));
        if (backend.isEventTriggered(want)) {
            want.eventTrigger.eventFilters = { topic: "projects/p/topics/t2" };
        }
        (0, chai_1.expect)(planner.changedV2PubSubTopic(want, have)).to.be.true;
    });
    it("detects upgrades to scheduled functions", () => {
        const v1Https = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv1" });
        const v1Scheduled = Object.assign(Object.assign({}, func("id", "region", { scheduleTrigger: {} })), { platform: "gcfv1" });
        const v2Https = Object.assign(Object.assign({}, func("id", "region")), { platform: "gcfv2" });
        const v2Scheduled = Object.assign(Object.assign({}, func("id", "region", { scheduleTrigger: {} })), { platform: "gcfv2" });
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v1Https, v1Https)).to.be.false;
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v2Https, v1Https)).to.be.false;
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v1Scheduled, v1Scheduled)).to.be.false;
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v2Scheduled, v2Scheduled)).to.be.false;
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v2Scheduled, v1Https)).to.be.false;
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v2Https, v1Scheduled)).to.be.false;
        (0, chai_1.expect)(planner.upgradedScheduleFromV1ToV2(v2Scheduled, v1Scheduled)).to.be.true;
    });
});
//# sourceMappingURL=planner.spec.js.map