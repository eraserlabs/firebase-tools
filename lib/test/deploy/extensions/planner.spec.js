...
describe("Extensions Deployment Planner", () => {
    describe("resolveSemver", () => {
        ...
        for (const c of cases) {
            it(c.description, () => {
                if (!c.err) {
                    (0, chai_1.expect)(planner.resolveVersion({
                        publisherId: "test",
                        extensionId: "test",
                        version: c.in,
    ...
    describe("have", () => {
        ...
        const INSTANCE_SPEC_WITH_UNDEFINED_EVENTS_CONFIG = JSON.parse(JSON.stringify(INSTANCE_SPEC_WITH_EVENTS));
        ...
        const INSTANCE_SPEC_WITH_EMPTY_EVENTS_CONFIG = JSON.parse(JSON.stringify(INSTANCE_SPEC_WITH_EVENTS));
        ...
        for (const c of cases) {
            it(c.description, async () => {
                ...
                (0, chai_1.expect)(have).to.have.same.deep.members(c.instanceSpecs);
