...
describe("encodeFirestoreValue", () => {
    it("should encode known types", () => {
        ...
        for (const test of tests) {
            (0, chai_1.expect)((0, encodeFirestoreValue_1.encodeFirestoreValue)(test.in)).to.deep.equal(test.res);
        }
    });
    it("should throw an error with unknown types", () => {
        ...
        for (const test of tests) {
            (0, chai_1.expect)(() => (0, encodeFirestoreValue_1.encodeFirestoreValue)(test.in)).to.throw(error_1.FirebaseError);
        }
    });
});
