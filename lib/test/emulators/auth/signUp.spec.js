...
(0, setup_1.describeAuthEmulator)("accounts:signUp", ({ authApi }) => {
    it("should throw error if no email provided", async () => {
        await authApi()
            ...
            (0, chai_1.expect)(res.body.error).to.have.property("message").equals("MISSING_EMAIL");
        });
    });
    it("should issue idToken and refreshToken on anon signUp", async () => {
        await authApi()
            ...
            (0, chai_1.expect)(decoded.payload.provider_id).equals("anonymous");
    ...
    it("should ignore displayName and photoUrl for new anon account", async () => {
        ...
        const idToken = await authApi()
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(200, res);
            (0, chai_1.expect)(res.body.displayName).to.be.undefined;
            (0, chai_1.expect)(res.body.photoUrl).to.be.undefined;
    ...
    it("should set displayName but ignore photoUrl for new password account", async () => {
        ...
        const idToken = await authApi()
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(200, res);
            (0, chai_1.expect)(res.body.displayName).to.equal(user.displayName);
            (0, chai_1.expect)(res.body.photoUrl).to.be.undefined;
            return res.body.idToken;
        });
        const info = await (0, helpers_1.getAccountInfoByIdToken)(authApi(), idToken);
        (0, chai_1.expect)(info.displayName).to.equal(user.displayName);
        (0, chai_1.expect)(info.photoUrl).to.be.undefined;
    });
    ...
    it("should error if another account exists with same email from IDP", async () => {
        ...
        await (0, helpers_1.signInWithFakeClaims)(authApi(), "google.com", { sub: "123", email });
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .send({ email, password: "notasecret" })
            .query({ key: "fake-api-key" })
    ...
    it("should error when email format is invalid", async () => {
        await authApi()
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error).to.have.property("message").equals("INVALID_EMAIL");
        });
    });
    it("should normalize email address to all lowercase", async () => {
        await authApi()
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(200, res);
            (0, chai_1.expect)(res.body.email).equals("alice@example.com");
        });
    });
    it("should error when password is too short", async () => {
        await authApi()
            ...
            (0, chai_1.expect)(res.body.error)
                ...
                .that.satisfy((str) => str.startsWith("WEAK_PASSWORD"));
        });
    });
    it("should error when idToken is provided but email / password is not", async () => {
        ...
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .send({ idToken, email: "alice@example.com" })
            .query({ key: "fake-api-key" })
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error).to.have.property("message").equals("MISSING_PASSWORD");
        });
    });
    ...
    it("should link email and password to phone sign-in user", async () => {
        ...
        const email = "alice@example.com";
        const { idToken, localId } = await (0, helpers_1.signInWithPhoneNumber)(authApi(), phoneNumber);
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
    ...
    it("should replace existing email / password in linked account", async () => {
        ...
        const { idToken, localId } = await (0, helpers_1.registerUser)(authApi(), {
            email: oldEmail,
        ...
        const oldEmailSignInMethods = await (0, helpers_1.getSigninMethods)(authApi(), oldEmail);
        (0, chai_1.expect)(oldEmailSignInMethods).to.be.empty;
    });
    it("should create new account with phone number when authenticated", async () => {
        ...
        const localId = await authApi()
            ...
            (0, chai_1.expect)(res.body.displayName).to.equal(displayName);
            (0, chai_1.expect)(res.body.localId).to.be.a("string").and.not.empty;
            return res.body.localId;
        });
        const phoneAuth = await (0, helpers_1.signInWithPhoneNumber)(authApi(), phoneNumber);
        (0, chai_1.expect)(phoneAuth.localId).to.equal(localId);
        const info = await (0, helpers_1.getAccountInfoByIdToken)(authApi(), phoneAuth.idToken);
        (0, chai_1.expect)(info.displayName).to.equal(displayName);
    });
    it("should error when extra localId parameter is provided", async () => {
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .query({ key: "fake-api-key" })
            .send({ localId: "anything" })
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error.message).to.equal("UNEXPECTED_PARAMETER : User ID");
        });
        const { idToken, localId } = await (0, helpers_1.registerAnonUser)(authApi());
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
    ...
    it("should error when creating new user with duplicate localId", async () => {
        ...
        await authApi()
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error.message).to.equal("DUPLICATE_LOCAL_ID");
        });
    });
    it("should error if phone number is invalid", async () => {
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .set("Authorization", "Bearer owner")
            .send({ phoneNumber: helpers_1.TEST_INVALID_PHONE_NUMBER })
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error.message).to.equal("INVALID_PHONE_NUMBER : Invalid format.");
        });
    });
    it("should create new account with multi factor info", async () => {
        const user = { email: "alice@example.com", password: "notasecret", mfaInfo: [helpers_1.TEST_MFA_INFO] };
        ...
        (0, chai_1.expect)(savedMfaInfo).to.include(helpers_1.TEST_MFA_INFO);
    ...
    it("should create new account with two MFA factors", async () => {
        const user = {
            ...
            password: "notasecret",
            mfaInfo: [helpers_1.TEST_MFA_INFO, Object.assign(Object.assign({}, helpers_1.TEST_MFA_INFO), { phoneInfo: helpers_1.TEST_PHONE_NUMBER_2 })],
        };
        const { localId } = await (0, helpers_1.registerUser)(authApi(), user);
        const info = await (0, helpers_1.getAccountInfoByLocalId)(authApi(), localId);
        (0, chai_1.expect)(info.mfaInfo).to.have.length(2);
        for (const savedMfaInfo of info.mfaInfo) {
            if (savedMfaInfo.phoneInfo !== helpers_1.TEST_MFA_INFO.phoneInfo) {
                (0, chai_1.expect)(savedMfaInfo.phoneInfo).to.eq(helpers_1.TEST_PHONE_NUMBER_2);
            }
            else {
                (0, chai_1.expect)(savedMfaInfo).to.include(helpers_1.TEST_MFA_INFO);
            }
    ...
    it("should de-duplicate factors with the same info on create", async () => {
        const alice = {
            email: "alice@example.com",
            password: "notasecret",
            mfaInfo: [helpers_1.TEST_MFA_INFO, helpers_1.TEST_MFA_INFO, helpers_1.TEST_MFA_INFO],
        };
        const { localId: aliceLocalId } = await (0, helpers_1.registerUser)(authApi(), alice);
        const aliceInfo = await (0, helpers_1.getAccountInfoByLocalId)(authApi(), aliceLocalId);
        (0, chai_1.expect)(aliceInfo.mfaInfo).to.have.length(1);
        (0, chai_1.expect)(aliceInfo.mfaInfo[0]).to.include(helpers_1.TEST_MFA_INFO);
        (0, chai_1.expect)(aliceInfo.mfaInfo[0].mfaEnrollmentId).to.be.a("string").and.not.empty;
        const bob = {
            ...
            mfaInfo: [
                ...
                helpers_1.TEST_MFA_INFO,
                helpers_1.TEST_MFA_INFO,
                Object.assign(Object.assign({}, helpers_1.TEST_MFA_INFO), { phoneInfo: helpers_1.TEST_PHONE_NUMBER_2 }),
            ],
        };
        const { localId: bobLocalId } = await (0, helpers_1.registerUser)(authApi(), bob);
        const bobInfo = await (0, helpers_1.getAccountInfoByLocalId)(authApi(), bobLocalId);
        (0, chai_1.expect)(bobInfo.mfaInfo).to.have.length(2);
    ...
    it("does not require a display name for multi factor info", async () => {
        const mfaInfo = { phoneInfo: helpers_1.TEST_PHONE_NUMBER };
        const user = { email: "alice@example.com", password: "notasecret", mfaInfo: [mfaInfo] };
        const { localId } = await (0, helpers_1.registerUser)(authApi(), user);
        const info = await (0, helpers_1.getAccountInfoByLocalId)(authApi(), localId);
        (0, chai_1.expect)(info.mfaInfo).to.have.length(1);
        const savedMfaInfo = info.mfaInfo[0];
        (0, chai_1.expect)(savedMfaInfo).to.include(mfaInfo);
        (0, chai_1.expect)(savedMfaInfo.mfaEnrollmentId).to.be.a("string").and.not.empty;
        (0, chai_1.expect)(savedMfaInfo.displayName).to.be.undefined;
    });
    it("should error if multi factor phone number is invalid", async () => {
        const mfaInfo = { phoneInfo: helpers_1.TEST_INVALID_PHONE_NUMBER };
        const user = { email: "alice@example.com", password: "notasecret", mfaInfo: [mfaInfo] };
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .set("Authorization", "Bearer owner")
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error.message).to.equal("INVALID_MFA_PHONE_NUMBER : Invalid format.");
        });
    });
    it("should ignore if multi factor enrollment ID is specified on create", async () => {
        ...
        const user = {
            ...
            password: "notasecret",
            mfaInfo: [
                Object.assign(Object.assign({}, helpers_1.TEST_MFA_INFO), { mfaEnrollmentId: mfaEnrollmentId1 }),
                Object.assign(Object.assign({}, helpers_1.TEST_MFA_INFO), { mfaEnrollmentId: mfaEnrollmentId2 }),
            ],
        };
        const { localId } = await (0, helpers_1.registerUser)(authApi(), user);
        const info = await (0, helpers_1.getAccountInfoByLocalId)(authApi(), localId);
        ...
        (0, chai_1.expect)(savedMfaInfo).to.include(helpers_1.TEST_MFA_INFO);
        (0, chai_1.expect)(savedMfaInfo.mfaEnrollmentId).to.be.a("string").and.not.empty;
        (0, chai_1.expect)([mfaEnrollmentId1, mfaEnrollmentId2]).not.to.include(savedMfaInfo.mfaEnrollmentId);
    });
    ...
    it("should error if password sign up is not allowed", async () => {
        const tenant = await (0, helpers_1.registerTenant)(authApi(), setup_1.PROJECT_ID, { allowPasswordSignup: false });
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .query({ key: "fake-api-key" })
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error).to.have.property("message").includes("OPERATION_NOT_ALLOWED");
        });
    });
    it("should error if anonymous user is disabled", async () => {
        const tenant = await (0, helpers_1.registerTenant)(authApi(), setup_1.PROJECT_ID, { enableAnonymousUser: false });
        await authApi()
            .post("/identitytoolkit.googleapis.com/v1/accounts:signUp")
            .query({ key: "fake-api-key" })
            ...
            .then((res) => {
            (0, helpers_1.expectStatusCode)(400, res);
            (0, chai_1.expect)(res.body.error).to.have.property("message").includes("ADMIN_ONLY_OPERATION");
        });
    });
    it("should create new account with tenant info", async () => {
        const tenant = await (0, helpers_1.registerTenant)(authApi(), setup_1.PROJECT_ID, { allowPasswordSignup: true });
        ...
        const info = await (0, helpers_1.getAccountInfoByLocalId)(authApi(), localId, tenant.tenantId);
        (0, chai_1.expect)(info.tenantId).to.eql(tenant.tenantId);
    });
    describe("when blocking functions are present", () => {
        ...
        it("should update modifiable fields with beforeCreate response for a new email/password user", async () => {
            ...
            await authApi()
                ...
                (0, chai_1.expect)(res.body.displayName).to.equal(helpers_1.DISPLAY_NAME);
